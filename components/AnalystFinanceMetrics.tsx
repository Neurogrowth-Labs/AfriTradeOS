...
// --- DATA ---
const FX_LIVE = [
  { pair: 'NGN/USD', rate: 1650.25, change: -2.5, bid: 1648.5, ask: 1652.0, spread: 3.5 },
  { pair: 'KES/USD', rate: 152.8, change: 1.2, bid: 152.6, ask: 153.0, spread: 0.4 },
  { pair: 'ZAR/USD', rate: 18.45, change: -0.8, bid: 18.43, ask: 18.47, spread: 0.04 },
  { pair: 'GHS/USD', rate: 14.92, change: -1.1, bid: 14.9, ask: 14.94, spread: 0.04 },
  { pair: 'EGP/USD', rate: 48.5, change: -3.2, bid: 48.4, ask: 48.6, spread: 0.2 },
  { pair: 'XOF/USD', rate: 605.5, change: 0.3, bid: 605.0, ask: 606.0, spread: 1.0 },
];
...
const FINANCE_PROVIDERS: FinanceProvider[] = [
  {
    ...
    minScore: 65,
    description:
      'Pan-African banking group with presence in 33 countries. Strong trade finance and SME solutions.',
  ...
  },
  {
    ...
    minScore: 70,
    description:
      "Africa's largest bank by assets. Strong in commodity finance and structured trade.",
...
export const AnalystFinanceMetrics: React.FC = () => {
  ...
  const totalLanded =
    calcData.cifValue + afcftaDuty + vat + calcData.shippingCost + calcData.insuranceCost;
  ...
  const handleCompare = (productType: string, providerId: string, minScore: number) => {
    if (userEligibilityScore < minScore) {
      setShowEligibilityWarning(
        `Your eligibility score (${userEligibilityScore}) is below the minimum required (${minScore}) for this provider.`
      );
  ...
  const handleApply = (providerId: string, productId: string, minScore: number) => {
    if (userEligibilityScore < minScore) {
      setShowEligibilityWarning(
        `Your eligibility score (${userEligibilityScore}) is below the minimum required (${minScore}) to apply.`
      );
      setTimeout(() => setShowEligibilityWarning(null), 4000);
      return;
    }
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">
                FX rates • Trade finance products • Credit risk • Profitability calculator
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
            ...
            <span
              className={`text-sm font-black ${userEligibilityScore >= 70 ? 'text-green-600' : 'text-amber-600'}`}
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-purple-200">
                  Based on your trade profile, sector, and eligibility score
                </p>
              </div>
            </div>
                ...
                <div
                  ...
                  className={`bg-white dark:bg-slate-800 rounded-xl border p-4 transition-all ${
                    provider.aiRecommended
                      ? 'border-purple-300 dark:border-purple-600 ring-1 ring-purple-100'
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {provider.specialties.map(spec => (
                      <span
                        key={spec}
                        className="text-[9px] bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 px-1.5 py-0.5 rounded"
                      >
                        {spec}
                      </span>
                    ))}
                  ...
                  <button
                    ...
                    title={
                      !isEligible
                        ? `Your score (${userEligibilityScore}) is below minimum (${provider.minScore})`
                ...
                <BarChart data={COUNTRY_RISK}>
                  ...
                  <Bar dataKey="credit" name="Credit Score" radius={[4, 4, 0, 0]} barSize={24}>
                    {COUNTRY_RISK.map((c, i) => (
                      <Cell
                        key={i}
                        fill={c.credit >= 70 ? '#10b981' : c.credit >= 60 ? '#f59e0b' : '#ef4444'}
                      />
                    ))}
                  </Bar>
                </BarChart>
                ...
                {COUNTRY_RISK.map(c => (
                  <tr key={c.country} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-black ${c.credit >= 70 ? 'text-green-600' : c.credit >= 60 ? 'text-amber-600' : 'text-red-600'}`}
                      >
                        {c.credit}/100
                      </span>
                    </td>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 overflow-y-auto">
            ...
            <div className="space-y-4">
              {[
                ...
                },
                {
                  ...
                  key: 'insuranceCost' as const,
                  min: 0,
                  max: 5000,
                  step: 50,
                },
                {
                  label: 'AfCFTA Duty Reduction (%)',
                  key: 'afcftaDiscount' as const,
                  min: 0,
                  max: 100,
                  step: 5,
                },
                ...
                <div key={input.key}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-black text-trade-primary dark:text-white">
                      {input.key === 'cifValue' ||
                      input.key === 'shippingCost' ||
                      input.key === 'insuranceCost'
                  ...
                  <input
                    ...
                    onChange={e =>
                      setCalcData(p => ({ ...p, [input.key]: Number(e.target.value) }))
                    }
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-gradient-to-br from-emerald-600 to-green-700 rounded-xl p-5 text-white">
              <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-200 mb-3">
                Profit After All Costs
              </h3>
              <p className="text-3xl font-black">
                ${totalLanded.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
                ...
                <div key={item.label}>
                  ...
                  <div className="h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(item.value / totalLanded) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-200 dark:border-green-800 p-4">
              ...
              <p className="text-2xl font-black text-green-600">
                ${dutySaving.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
              <p className="text-[10px] text-green-600">
                Duty reduction from ${duty.toFixed(0)} → ${afcftaDuty.toFixed(0)} (
                {calcData.afcftaDiscount}% reduction)
              </p>
            </div>
          </div>
        </div>
