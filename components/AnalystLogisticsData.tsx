...
const CORRIDORS: CorridorScore[] = [
  ...
  {
    id: 'cs2',
    name: 'Northern Corridor (Mombasa–Kampala)',
    score: 71,
    clearanceTime: '48h',
    infra: 68,
    border: 60,
  ...
  {
    id: 'cs3',
    name: 'Trans-Sahara (Lagos–Algiers)',
    score: 42,
    clearanceTime: '96h',
    infra: 35,
    border: 40,
...
const WAREHOUSES: WarehouseData[] = [
  {
    id: 'wh1',
    name: 'Lagos Free Zone Warehouse A',
    ...
    features: ['24/7 Security', 'CCTV', 'Fire Suppression', 'Loading Docks'],
    contact: { phone: '+234 1 234 5678', email: 'lagos@bollore.com' },
    x: 320,
    y: 350,
  },
  {
    id: 'wh2',
    name: 'Tema Port Logistics Hub',
    operator: 'Maersk Warehousing',
    country: 'Ghana',
    city: 'Tema',
    type: 'general',
    capacity: '18,000 sqm',
    ...
    features: ['Rail Access', 'Container Yard', 'Cross-Docking'],
    contact: { phone: '+233 30 212 3456', email: 'tema@maersk.com' },
    x: 280,
    y: 330,
  },
  {
    id: 'wh3',
    name: 'Mombasa Cold Chain Center',
    operator: 'Kuehne+Nagel',
    country: 'Kenya',
    city: 'Mombasa',
    type: 'cold_storage',
    capacity: '8,000 sqm',
    ...
    rating: 4.7,
    reviews: 67,
    certifications: ['HACCP', 'GDP', 'ISO 22000'],
    features: ['-25°C to +15°C', 'Pharma Grade', 'Reefer Plugs'],
    contact: { phone: '+254 41 234 5678', email: 'mombasa@kuehne-nagel.com' },
    x: 600,
    y: 460,
  },
  {
    ...
    rating: 4.5,
    reviews: 156,
    certifications: ['ISO 9001', 'BEE Level 2'],
    features: ['Automated WMS', 'E-commerce Fulfillment', 'Value-Added Services'],
    contact: { phone: '+27 31 234 5678', email: 'durban@imperiallogistics.com' },
    x: 520,
    y: 720,
  },
  {
    id: 'wh5',
    name: 'Djibouti Free Trade Zone',
    operator: 'DP World',
    country: 'Djibouti',
    city: 'Djibouti City',
    type: 'bonded',
    ...
    rating: 4.4,
    reviews: 45,
    certifications: ['ISO 28000', 'AEO'],
    features: ['Duty Free', 'Re-export Hub', 'Multi-modal'],
    contact: { phone: '+253 21 35 1234', email: 'djibouti@dpworld.com' },
    x: 650,
    y: 310,
  },
  {
    id: 'wh6',
    name: 'Tangier Med Logistics Platform',
    operator: 'TMSA',
    country: 'Morocco',
    city: 'Tangier',
    type: 'bonded',
    ...
    features: ['EU Gateway', 'Automotive Hub', 'Pharma Zone'],
    contact: { phone: '+212 539 33 7000', email: 'logistics@tmsa.ma' },
    x: 270,
    y: 80,
  },
  {
    id: 'wh7',
    name: 'Cairo Hazmat Storage Facility',
    operator: 'Agility',
    country: 'Egypt',
    city: 'Cairo',
    type: 'hazmat',
    ...
    rating: 4.3,
    reviews: 34,
    certifications: ['ADR', 'IMDG', 'ISO 45001'],
    features: ['Hazmat Licensed', 'Spill Containment', 'Emergency Response'],
    contact: { phone: '+20 2 2345 6789', email: 'cairo@agility.com' },
    x: 560,
    y: 100,
  },
  {
    ...
    features: ['Inland Container Depot', 'Customs Clearance', 'Transit Storage'],
    contact: { phone: '+255 22 211 2345', email: 'dar@bollore.com' },
    x: 610,
    y: 510,
  },
];
...
export const AnalystLogisticsData: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'ports' | 'routes' | 'freight' | 'corridors' | 'warehouses'
  >('ports');
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">
                Port congestion • Route optimization • Freight estimator • Corridor scoring •
                Warehouse locator
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-0">
            ...
            <svg viewBox="0 0 800 800" className="w-full h-full">
              ...
              {PORTS.map(p => {
                ...
                return (
                  <g
                    key={p.id}
                    onClick={() => setSelectedPort(selectedPort?.id === p.id ? null : p)}
          ...
          <div className="lg:col-span-5 flex flex-col gap-3 overflow-y-auto">
            {PORTS.map(p => (
              <div
                key={p.id}
                className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-3 hover:shadow-md transition-shadow cursor-pointer"
              ...
              >
                ...
                <div className="flex items-center justify-between text-[10px]">
                  <span className="text-gray-500">
                    Wait:{' '}
                    <span className="font-bold text-gray-800 dark:text-white">{p.waitTime}</span>
                  </span>
                  <span className="text-gray-500">{p.throughput}</span>
                  <span
                    className={`font-bold ${p.trend.startsWith('+') ? 'text-green-600' : 'text-red-500'}`}
                  >
                    {p.trend}
                  </span>
                </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-2">
                      ...
                      <p
                        className={`text-sm font-black ${r.reliability >= 80 ? 'text-green-600' : r.reliability >= 70 ? 'text-amber-600' : 'text-red-600'}`}
                      >
                        {r.reliability}%
                      </p>
                    </div>
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-5">
            <h3 className="text-sm font-bold text-trade-primary dark:text-white mb-4 flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-green-500" /> Freight Cost Comparison (USD per
              Container)
            </h3>
                ...
                <BarChart data={CORRIDORS} layout="vertical">
                  ...
                  <Bar dataKey="score" name="Score" radius={[0, 6, 6, 0]} barSize={16}>
                    {CORRIDORS.map((c, i) => (
                      <Cell
                        key={i}
                        fill={c.score >= 80 ? '#10b981' : c.score >= 60 ? '#f59e0b' : '#ef4444'}
                      />
                    ))}
                  </Bar>
                </BarChart>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    <span
                      className={`text-lg font-black ${c.score >= 80 ? 'text-green-600' : c.score >= 60 ? 'text-amber-600' : 'text-red-600'}`}
                    >
                      {c.score}
                    </span>
                ...
                {filteredWarehouses.map(w => {
                  ...
                  return (
                    <g
                      key={w.id}
                      onClick={() =>
                        setSelectedWarehouse(selectedWarehouse?.id === w.id ? null : w)
                      }
                      className="cursor-pointer"
                    >
                ...
                <div className="absolute bottom-3 left-3 right-3 md:right-auto md:w-80 bg-white/95 dark:bg-slate-800/95 backdrop-blur rounded-xl p-4 border shadow-2xl z-20">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">
                        {selectedWarehouse.operator} • {selectedWarehouse.city},{' '}
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {selectedWarehouse.features.map(f => (
                      <span
                        key={f}
                        className="text-[9px] bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 px-2 py-0.5 rounded-full"
                      >
                        {f}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-2">
                    <a
                      href={`tel:${selectedWarehouse.contact.phone}`}
                      className="flex-1 py-2 bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1"
                    >
                      <Phone className="w-3 h-3" /> Call
                    </a>
                    <a
                      href={`mailto:${selectedWarehouse.contact.email}`}
                      className="flex-1 py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1"
                    >
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="text-center p-1.5 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
                        <p className="text-[9px] text-gray-400">Capacity</p>
                        <p className="text-[10px] font-bold text-gray-800 dark:text-white">
                          {w.capacity}
                        </p>
                      </div>
                      <div className="text-center p-1.5 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
                        <p className="text-[9px] text-gray-400">Available</p>
                        <p
                          className={`text-[10px] font-bold ${w.available >= 50 ? 'text-green-600' : w.available >= 25 ? 'text-amber-600' : 'text-red-600'}`}
                        >
                          {w.available}%
                        </p>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {w.certifications.slice(0, 3).map(cert => (
                        <span
                          key={cert}
                          className="text-[8px] bg-blue-50 dark:bg-blue-900/20 text-blue-600 px-1.5 py-0.5 rounded"
                        >
                          {cert}
                        </span>
                      ))}
                      {w.certifications.length > 3 && (
                        <span className="text-[8px] text-gray-400">
                          +{w.certifications.length - 3} more
                        </span>
                      )}
                    </div>
                  </div>
