...
const MARKET_PLAYERS: MarketPlayer[] = [
  {
    ...
    rating: 4.8,
    reviews: 124,
    certifications: ['AfCFTA Certified', 'ISO 9001', 'Fair Trade'],
    description:
      'Premier cocoa supplier with vertically integrated processing. 60+ years of trade excellence.',
    logo: 'G',
    tags: ['cocoa', 'agriculture', 'premium'],
    eligibilityScore: 88,
  },
  {
    id: 'mp2',
    ...
    rating: 4.6,
    reviews: 89,
    certifications: ['AfCFTA Certified', 'ISO 14001'],
    description:
      "Africa's largest industrial conglomerate. Major buyer of raw materials and commodities.",
    logo: 'D',
    tags: ['cement', 'manufacturing', 'bulk'],
    eligibilityScore: 92,
  },
  {
    id: 'mp3',
    name: 'Maersk East Africa',
    type: 'logistics',
    country: 'Kenya',
    sector: 'Logistics',
    trustScore: 88,
    ...
    rating: 4.5,
    reviews: 256,
    certifications: ['ISO 28000', 'AEO Certified'],
    description: 'Full container and LCL shipping across African corridors. Digital tracking.',
    logo: 'M',
    tags: ['shipping', 'container', 'sea freight'],
    eligibilityScore: 85,
  },
  {
    id: 'mp4',
    name: 'KCB Trade Finance',
    type: 'finance',
    country: 'Kenya',
    sector: 'Banking',
    trustScore: 93,
    ...
    rating: 4.4,
    reviews: 67,
    certifications: ['Central Bank Licensed', 'AfCFTA Trade Finance'],
    description:
      'Leading East African trade finance provider. L/C, guarantees, and supply chain finance.',
    logo: 'K',
  ...
  },
  {
    ...
    rating: 4.3,
    reviews: 198,
    certifications: ['ISO 9001', 'TAPA'],
    description:
      'Pan-African logistics operator with port terminal operations across 20+ countries.',
    logo: 'B',
    tags: ['logistics', 'ports', 'warehousing'],
    eligibilityScore: 80,
  },
  {
    id: 'mp6',
    ...
    rating: 4.7,
    reviews: 78,
    certifications: ['AfCFTA Certified', 'Organic', 'Rainforest Alliance'],
    description: 'Specialty coffee exporter. Direct sourcing from Ethiopian highlands.',
    logo: 'E',
  ...
  },
  {
    ...
    rating: 4.9,
    reviews: 45,
    certifications: ['AAA Rated', 'DFI'],
    description: 'African Export-Import Bank. Continental trade finance institution.',
    logo: 'A',
    tags: ['DFI', 'export finance', 'guarantees'],
    eligibilityScore: 95,
  },
  {
    id: 'mp8',
    ...
    description:
      "Africa's largest food retailer. Sourcing fresh produce and FMCG across 15 countries.",
    logo: 'S',
    tags: ['retail', 'FMCG', 'fresh produce'],
    eligibilityScore: 82,
  },
];
...
export const AnalystMarketPlayers: React.FC = () => {
  ...
  const handleNavigate = (path: string, minScore?: number, actionName?: string) => {
    if (minScore && userEligibilityScore < minScore) {
      setShowEligibilityWarning(
        `Your eligibility score (${userEligibilityScore}) is below the minimum required (${minScore}) for ${actionName || 'this action'}.`
      );
  ...
  const goToPrepareBid = (playerId?: string) =>
    handleNavigate(
      `/bids/prepare${playerId ? `?partner=${playerId}` : ''}`,
      MIN_ELIGIBILITY_SCORE,
      'Prepare Bid'
    );
  const goToMessage = (playerId: string) =>
    handleNavigate(`/messages/inbox?to=${playerId}`, MIN_ELIGIBILITY_SCORE, 'Message');
  ...
  return (
    <div className="h-full flex flex-col gap-4 animate-fade-in pb-4">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">
                Verified directory • Trust scores • AI matchmaking • Business profiles
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={
                userEligibilityScore < MIN_ELIGIBILITY_SCORE
                  ? `Minimum score required: ${MIN_ELIGIBILITY_SCORE}`
                  : 'Prepare a new bid'
              }
            >
        ...
        <div className="flex items-center gap-4 mb-3 p-2 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-sm font-black ${userEligibilityScore >= MIN_ELIGIBILITY_SCORE ? 'text-green-600' : 'text-amber-600'}`}
            >
          ...
          {userEligibilityScore >= MIN_ELIGIBILITY_SCORE ? (
            ...
            <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> Some actions restricted (Min:{' '}
              {MIN_ELIGIBILITY_SCORE})
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-indigo-200">
              Match scores calculated based on your trade profile, sector, and location
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-0 overflow-hidden">
        {/* Grid */}
        <div
          className={`${selectedPlayer ? 'lg:col-span-7' : 'lg:col-span-12'} overflow-y-auto space-y-3`}
        >
          {filtered.map(player => {
            ...
            return (
              <div
                ...
                className={`bg-white dark:bg-slate-800 rounded-xl border p-4 cursor-pointer transition-all hover:shadow-md ${
                  selectedPlayer?.id === player.id
                    ? 'border-indigo-400 dark:border-indigo-600 ring-1 ring-indigo-200'
              ...
              >
                <div className="flex items-start gap-3">
                  {/* Avatar */}
                  <div
                    className={`w-12 h-12 rounded-xl ${config.bg} flex items-center justify-center text-lg font-black ${config.color} shrink-0`}
                  >
                    {player.logo}
                  </div>
          ...
          <div className="lg:col-span-5 bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 overflow-y-auto">
            <div className="p-5 border-b border-gray-100 dark:border-slate-700">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-14 h-14 rounded-xl ${TYPE_CONFIG[selectedPlayer.type].bg} flex items-center justify-center text-xl font-black ${TYPE_CONFIG[selectedPlayer.type].color}`}
                  >
                    {selectedPlayer.logo}
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {selectedPlayer.country} • {selectedPlayer.sector}
                    </p>
                  </div>
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedPlayer.certifications.map(cert => (
                    <span
                      key={cert}
                      className="text-[10px] bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-2 py-1 rounded-lg flex items-center gap-1"
                    >
              ...
              <div className="mb-4">
                ...
                <span
                  className={`text-xs font-bold px-3 py-1 rounded-lg flex items-center gap-1.5 w-fit ${
                    selectedPlayer.kycStatus === 'verified'
                      ? 'bg-green-100 text-green-700'
                      : selectedPlayer.kycStatus === 'pending'
              ...
              {selectedPlayer.eligibilityScore && (
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs font-bold ${selectedPlayer.eligibilityScore >= 80 ? 'text-green-600' : selectedPlayer.eligibilityScore >= 60 ? 'text-amber-600' : 'text-red-600'}`}
                    >
                      {selectedPlayer.eligibilityScore}%
                    </span>
                  </div>
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  title={
                    userEligibilityScore < MIN_ELIGIBILITY_SCORE
                      ? `Minimum score required: ${MIN_ELIGIBILITY_SCORE}`
                      : 'Prepare a bid for this partner'
                  }
                >
