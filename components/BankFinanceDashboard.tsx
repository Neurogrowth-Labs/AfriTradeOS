...
const CREDIT_PIPELINE: CreditApplication[] = [
  ...
  {
    id: 'APP002',
    applicant: 'Lagos Trading Co',
    type: 'Export Factoring',
    amount: 1200000,
    currency: 'USD',
    stage: 'under_review',
  ...
  {
    id: 'APP003',
    applicant: 'Accra Commodities',
    type: 'Trade Guarantee',
    amount: 750000,
    currency: 'USD',
    stage: 'kyc_hold',
...
const RISK_ALERTS: RiskAlert[] = [
  {
    ...
    type: 'shipment',
    severity: 'high',
    title: 'Shipment Stuck in Mombasa',
    description:
      'Container MSCU7234567 has been at Mombasa port for 5 days without movement. LC-2024-00890.',
    timestamp: '2024-02-14T10:30:00Z',
    actionRequired: true,
    relatedEntity: 'Ethiopian Coffee Co',
  },
  {
    id: 'RA002',
    type: 'aml',
    severity: 'critical',
    title: 'AML Score Increased',
    description:
      'Client "Cairo Imports" AML risk score increased from 45 to 72 due to new transaction patterns.',
    timestamp: '2024-02-14T09:15:00Z',
    actionRequired: true,
    relatedEntity: 'Cairo Imports',
  },
  {
    id: 'RA003',
    type: 'policy',
    ...
    description:
      'Trade credit insurance policy for "Lagos Trading Co" expires in 7 days. Renewal required.',
    timestamp: '2024-02-14T08:00:00Z',
    actionRequired: true,
    relatedEntity: 'Lagos Trading Co',
  },
  {
    id: 'RA004',
    type: 'fx',
    severity: 'high',
    title: 'Currency Risk Threshold Breached',
    description:
      'NGN/USD volatility exceeded 5% threshold. Current exposure: $12.5M in Naira-denominated trades.',
  ...
  },
  {
    ...
    description: 'Durban port experiencing 3-day average delays. 8 shipments potentially affected.',
  ...
  },
  {
    ...
    type: 'political',
    severity: 'low',
    title: 'Election Period Advisory',
    description:
      'Kenya general elections scheduled. Enhanced monitoring recommended for Kenyan corridor trades.',
  ...
  },
  {
    ...
    description:
      'Potential OFAC match for beneficial owner of "XYZ Trading Ltd". Immediate review required.',
...
export const BankFinanceDashboard: React.FC = () => {
  ...
  const [activeExposureView, setActiveExposureView] = useState<
    'country' | 'sector' | 'corridor' | 'currency'
  >('country');
  ...
  const [portfolioByCountry, setPortfolioByCountry] =
    useState<PortfolioExposure[]>(PORTFOLIO_BY_COUNTRY);
  const [portfolioBySection, setPortfolioBySector] =
    useState<PortfolioExposure[]>(PORTFOLIO_BY_SECTOR);
  const [portfolioByCorridor, setPortfolioByCorridor] =
    useState<PortfolioExposure[]>(PORTFOLIO_BY_CORRIDOR);
  ...
  const getExposureData = () => {
    switch (activeExposureView) {
      case 'country':
        return portfolioByCountry;
      case 'sector':
        return portfolioBySection;
      case 'corridor':
        return portfolioByCorridor;
      default:
        return portfolioByCountry;
    }
  };
  ...
  const filteredTradeFlows = tradeFlows.filter(flow => {
    const matchesCountry =
      filterCountry === 'all' || flow.corridor.toLowerCase().includes(filterCountry.toLowerCase());
  ...
  const utilizationRate =
    totalFacilities > 0 ? Math.round((outstandingFacilities / totalFacilities) * 100) : 78;
  ...
  const uniqueCorridors = Array.from(
    new Set([...portfolioByCorridor.map(p => p.category), ...tradeFlows.map(f => f.corridor)])
  )
    .filter(c => c && c !== 'Others')
    .sort();
    ...
    <div className="h-full flex flex-col gap-4 animate-fade-in pb-4 overflow-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Portfolio • Trade Flows • Risk Intelligence • Credit Pipeline
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700 shadow-sm">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-white">
            ${(outstandingFacilities / 1000000).toFixed(0)}M
          </p>
          <div className="flex items-center gap-1 mt-1">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-slate-700">
                    <th className="text-left text-[10px] font-bold text-gray-400 uppercase py-2 px-2">
                      Trade Ref
                    </th>
                    <th className="text-left text-[10px] font-bold text-gray-400 uppercase py-2 px-2">
                      Corridor
                ...
                <tbody>
                  {filteredTradeFlows.map(flow => (
                    <tr
                      key={flow.id}
                      className="border-b border-gray-50 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30 cursor-pointer transition-colors"
                    >
                      ...
                      <td className="py-2.5 px-2">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${getStatusColor(flow.status)}`}
                        >
                      ...
                      {stageApps.map(app => (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mt-1">
                            ...
                            <span
                              className={`text-[9px] ${app.riskScore >= 70 ? 'text-green-600' : app.riskScore >= 50 ? 'text-amber-600' : 'text-red-600'}`}
                            >
                              {app.riskScore}
                            </span>
                          </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-gray-500">
                {riskAlerts.filter(a => a.actionRequired).length} action required
              </span>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-4 flex-1">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-gray-500">
                {countryRisks.length} countries monitored
              </span>
            </div>
            <div className="space-y-2">
              {countryRisks
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[9px] text-gray-500">
                          FX: {country.fxVolatility}%
                        </span>
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded ${
                            country.politicalRisk === 'high'
                              ? 'bg-red-100 text-red-600 dark:bg-red-900/30'
                              : country.politicalRisk === 'medium'
