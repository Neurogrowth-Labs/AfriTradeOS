...
export const CustomsAuthorityPanel: React.FC = () => {
  ...
  const [kpis, setKpis] = useState({
    ...
    highRiskCount: 0,
    afcftaUtilization: 0,
    topCommodities: [] as any[],
    declarationsByStatus: [] as any[],
    declarationsByCountry: [] as any[],
    riskDistribution: [] as any[],
    revenueByType: [] as any[],
    officerPerformance: [] as any[],
  });
    ...
    const liveInterval = setInterval(() => {
      setKpis(prev => ({
        ...
        pendingReview: Math.max(
          0,
          prev.pendingReview + (Math.random() > 0.7 ? 1 : Math.random() > 0.5 ? -1 : 0)
        ),
        clearedToday: prev.clearedToday + (Math.random() > 0.8 ? 1 : 0),
        revenueToday:
          prev.revenueToday + (Math.random() > 0.6 ? Math.floor(Math.random() * 5000) : 0),
      }));
    }, 5000);
  ...
  const handleVerifyCertificate = async (certId: string) => {
    ...
    setCertificates(prev =>
      prev.map(c =>
        c.id === certId
          ? { ...c, status: 'verified' as const, verified_at: new Date().toISOString() }
          : c
      )
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <KPICard
          ...
          icon={<Clock />}
          color="amber"
          trend={kpis.pendingReview > 10 ? 'up' : 'down'}
        />
        <KPICard
          title="Cleared Today"
          value={kpis.clearedToday}
          icon={<CheckCircle />}
          color="green"
        />
        <KPICard
              ...
              {reviewQueue.slice(0, 5).map(decl => (
                <tr key={decl.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                  ...
                  <td className="py-2 px-3 text-emerald-400">
                    {formatCurrency(decl.declared_value)}
                  </td>
                  <td className="py-2 px-3">
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-bold ${
                        decl.risk_level === 'critical'
                          ? 'bg-red-500/20 text-red-400'
                          : decl.risk_level === 'high'
                            ? 'bg-orange-500/20 text-orange-400'
                            : decl.risk_level === 'medium'
              ...
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white text-sm">
                    {item.description?.substring(0, 30) || 'N/A'}
                  </span>
                </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-slate-400 text-xs">
                    {decl.product_description?.substring(0, 40)}...
                  </p>
                </div>
                <div className="text-right">
                  <span
          ...
          <div className="p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="text-xl font-bold text-white">
                  {selectedDeclaration.declaration_number}
                </h3>
            ...
            <div
              className={`p-4 rounded-lg mb-6 ${
                selectedDeclaration.risk_level === 'critical'
                  ? 'bg-red-900/30 border border-red-500'
                  : selectedDeclaration.risk_level === 'high'
                    ? 'bg-orange-900/30 border border-orange-500'
                    : selectedDeclaration.risk_level === 'medium'
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-white">
                      Risk Score: {selectedDeclaration.risk_score}/100
                    </p>
                    <p className="text-sm opacity-80">
                      Level: {selectedDeclaration.risk_level.toUpperCase()}
                    </p>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <DetailRow
                  label="Port of Entry"
                  value={selectedDeclaration.port_of_entry || 'N/A'}
                />
              </div>
              <div className="space-y-4">
                ...
                <DetailRow
                  label="Quantity"
                  value={`${selectedDeclaration.quantity} ${selectedDeclaration.unit}`}
                />
                <DetailRow
                  label="Declared Value"
                  value={formatCurrency(selectedDeclaration.declared_value)}
                  highlight
                />
              </div>
              <div className="space-y-4">
                ...
                <DetailRow
                  label="Duty Amount"
                  value={formatCurrency(selectedDeclaration.duty_amount)}
                />
                <DetailRow label="VAT" value={formatCurrency(selectedDeclaration.vat_amount)} />
                <DetailRow
                  label="Total Taxes"
                  value={formatCurrency(selectedDeclaration.total_taxes)}
                  highlight
                />
              </div>
              <div className="space-y-4">
                ...
                <DetailRow
                  ...
                  value={
                    selectedDeclaration.submitted_at
                      ? new Date(selectedDeclaration.submitted_at).toLocaleString()
                      : 'N/A'
                  }
                />
              </div>
          ...
          <div key={trader.id} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    trader.risk_classification === 'trusted'
                      ? 'bg-green-500/20 text-green-400'
                      : trader.risk_classification === 'high_risk'
                        ? 'bg-red-500/20 text-red-400'
                        : trader.risk_classification === 'blacklisted'
              ...
              {trader.aeo_tier && (
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold ${
                    trader.aeo_tier === 'gold'
                      ? 'bg-yellow-500/20 text-yellow-400'
                      : trader.aeo_tier === 'silver'
                        ? 'bg-slate-400/20 text-slate-300'
                        : 'bg-orange-500/20 text-orange-400'
                  }`}
                >
                  {trader.aeo_tier.toUpperCase()}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <p
                  className={`font-bold ${trader.compliance_score >= 80 ? 'text-green-400' : trader.compliance_score >= 60 ? 'text-yellow-400' : 'text-red-400'}`}
                >
              ...
              <div>
                ...
                <p className="text-emerald-400 font-bold">
                  {formatCurrency(trader.total_trade_value)}
                </p>
              </div>
              <div>
                <p className="text-slate-500">Violations</p>
                <p
                  className={`font-bold ${trader.violations_count > 0 ? 'text-red-400' : 'text-green-400'}`}
                >
            ...
            <div className="mt-3 pt-3 border-t border-slate-700 flex items-center justify-between">
              <span
                className={`px-2 py-0.5 rounded text-xs ${
                  trader.sanctions_status === 'clear'
                    ? 'bg-green-500/20 text-green-400'
                    : trader.sanctions_status === 'flagged'
                      ? 'bg-yellow-500/20 text-yellow-400'
                      : 'bg-red-500/20 text-red-400'
                }`}
              >
              ...
              <button
                onClick={() => handleViewTraderProfile(trader)}
              ...
              {declarations.map(decl => (
                <tr key={decl.id} className="border-t border-slate-700 hover:bg-slate-700/30">
                  ...
                  <td className="py-3 px-4 capitalize text-white">
                    {decl.declaration_type.replace('_', ' ')}
                  </td>
                  ...
                  <td className="py-3 px-4 text-slate-300">
                    {decl.origin_country} → {decl.destination_country}
                  </td>
                  ...
                  <td className="py-3 px-4 text-center">
                    <span
                      ...
                      style={{
                        backgroundColor: `${RISK_COLORS[decl.risk_level]}20`,
                        color: RISK_COLORS[decl.risk_level],
                      }}
                    >
                      {decl.risk_level}
                    </span>
                  ...
                  <td className="py-3 px-4 text-center">
                    <span
                      ...
                      style={{
                        backgroundColor: `${STATUS_COLORS[decl.status]}20`,
                        color: STATUS_COLORS[decl.status],
                      }}
                    >
                      {decl.status.replace('_', ' ')}
                    </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-2xl font-bold text-white">
                {certificates.filter(c => c.afcfta_compliant).length}
              </p>
        ...
        {['pre_arrival', 'arrived', 'under_inspection', 'customs_hold', 'cleared'].map(status => {
          ...
          return (
            <div key={status} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
              <div
                className={`w-10 h-10 rounded-lg bg-${colors[status]}-500/20 flex items-center justify-center mb-2 text-${colors[status]}-400`}
              >
                {icons[status]}
              </div>
          ...
          {shipments.map(shipment => (
            <div key={shipment.id} className="p-4 hover:bg-slate-700/30">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-medium">
                      {shipment.tracking_number || shipment.container_number}
                    </p>
                    <p className="text-slate-400 text-xs">
                      {shipment.carrier_name} • {shipment.vessel_name}
                    </p>
                  </div>
                </div>
                <span
                  className={`px-2 py-1 rounded text-xs font-bold capitalize ${
                    shipment.status === 'cleared'
                      ...
                      : shipment.status === 'customs_hold'
                        ? 'bg-red-500/20 text-red-400'
                        : shipment.status === 'under_inspection'
                          ? 'bg-amber-500/20 text-amber-400'
                          : 'bg-blue-500/20 text-blue-400'
                  }`}
                >
              ...
              <div className="flex items-center gap-6 text-sm">
                ...
                <div
                  className={`flex items-center gap-1 ${shipment.risk_score > 70 ? 'text-red-400' : shipment.risk_score > 40 ? 'text-amber-400' : 'text-green-400'}`}
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          ...
          <div className="space-y-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-slate-400 text-xs">
                      {officer.declarations_reviewed} reviewed
                    </span>
                  </div>
                  <div className="w-full bg-slate-700 rounded-full h-2">
                    <div
                      className="bg-purple-500 h-2 rounded-full"
                      style={{ width: `${officer.accuracy_rate}%` }}
                    />
                  </div>
                </div>
            ...
            {officers.length === 0 && (
              <p className="text-slate-500 text-center py-4">No officer data</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          {[
            ...
            >
              ...
              <span
                className={`px-2 py-1 rounded text-xs font-bold bg-${integration.color}-500/20 text-${integration.color}-400`}
              >
                {integration.status}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-sm">
              <div
                className={`w-2 h-2 rounded-full ${liveRefresh ? 'bg-green-500 animate-pulse' : 'bg-slate-500'}`}
              />
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center ${
                    selectedTrader.risk_classification === 'trusted'
                      ? 'bg-green-500/20 text-green-400'
                      : selectedTrader.risk_classification === 'high_risk'
                        ? 'bg-red-500/20 text-red-400'
                        : 'bg-slate-600 text-slate-300'
                  }`}
                >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-700/50 rounded-xl p-4 text-center">
                  ...
                  >
                    {selectedTrader.risk_classification.replace('_', ' ')}
                  </p>
                </div>
                <div className="bg-slate-700/50 rounded-xl p-4 text-center">
                  <p className="text-slate-400 text-xs uppercase mb-1">Compliance Score</p>
                  <p
                    className={`text-lg font-bold ${selectedTrader.compliance_score >= 80 ? 'text-green-400' : selectedTrader.compliance_score >= 60 ? 'text-amber-400' : 'text-red-400'}`}
                  >
                ...
                <div className="bg-slate-700/50 rounded-xl p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-white">
                    {selectedTrader.total_declarations}
                  </p>
                </div>
                <div className="bg-slate-700/50 rounded-xl p-4 text-center">
                  <p className="text-slate-400 text-xs uppercase mb-1">Trade Value</p>
                  <p className="text-lg font-bold text-emerald-400">
                    {formatCurrency(selectedTrader.total_trade_value)}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">AEO Tier</span>
                      <span className="text-white font-medium">
                        {selectedTrader.aeo_tier?.toUpperCase() || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Sanctions Status</span>
                      <span
                        className={`font-medium ${selectedTrader.sanctions_status === 'clear' ? 'text-green-400' : 'text-red-400'}`}
                      >
                        {selectedTrader.sanctions_status}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Violations</span>
                      <span
                        className={`font-medium ${selectedTrader.violations_count > 0 ? 'text-red-400' : 'text-green-400'}`}
                      >
                        {selectedTrader.violations_count}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Last Declaration</span>
                      <span className="text-white">
                        {(selectedTrader as any).last_declaration_date
                          ? new Date(
                              (selectedTrader as any).last_declaration_date
                            ).toLocaleDateString()
                          : 'N/A'}
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-500">Avg Clearance</span>
                      <span className="text-white">
                        {(selectedTrader as any).average_clearance_time || 'N/A'} hrs
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-700">
                <button
                  onClick={() => {
                    ...
                    setSearchQuery(selectedTrader.tin);
        ...
        <div className="fixed bottom-6 right-6 z-40">
          ...
          >
            ...
            <span className="font-bold">
              {liveAlertCount} New Alert{liveAlertCount > 1 ? 's' : ''}
            </span>
          </button>
        </div>
      )}
