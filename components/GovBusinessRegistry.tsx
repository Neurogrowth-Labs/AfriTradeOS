...
const FRAUD_ALERTS = [
  {
    entity: 'Unknown',
    alert: 'Linked to sanctioned entity via ownership chain',
    severity: 'high',
    date: '2024-12-18',
  },
];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setCompanies(
          orgs.map((o: Record<string, unknown>, i: number) => {
            ...
            return {
              ...
              traderTier:
                score >= 90 ? 'Gold' : score >= 70 ? 'Silver' : score >= 50 ? 'Bronze' : 'Standard',
              ...
              contacts: {
                ...
                email:
                  (o.email as string) ||
                  `info@${((o.name as string) || 'company').toLowerCase().replace(/\s/g, '')}.com`,
                website:
                  (o.website as string) ||
                  `${((o.name as string) || 'company').toLowerCase().replace(/\s/g, '')}.com`,
              },
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Single source of truth — company profiles, ownership, compliance scores & network
              intelligence
            </p>
          </div>
                ...
                {filteredCompanies.map(company => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400">
                            {company.country} &middot; {company.sector} &middot; {company.type}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="text-right">
                          <p
                            className={`text-sm font-black ${getScoreColor(company.complianceScore)}`}
                          >
                        ...
                        <span
                          className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${getTierColor(company.traderTier)}`}
                        >
                          {company.traderTier}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-[10px] text-gray-400">
                      ...
                      {company.permitsPending > 0 && (
                        <span className="text-amber-500 font-bold">
                          {company.permitsPending} pending
                        </span>
                      )}
                    </div>
                  </div>
              ...
              {selectedCompany ? (
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">
                          {selectedCompany.id} &middot; Est. {selectedCompany.founded}
                        </p>
                      </div>
                    </div>
                    <div className="space-y-2 text-xs mb-4">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 bg-gray-200 dark:bg-slate-700 rounded-full">
                            <div
                              className={`h-full rounded-full ${getScoreBg(selectedCompany.complianceScore)}`}
                              style={{ width: `${selectedCompany.complianceScore}%` }}
                            ></div>
                          </div>
                          <span
                            className={`font-black ${getScoreColor(selectedCompany.complianceScore)}`}
                          >
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold ${getTierColor(selectedCompany.traderTier)}`}
                        >
                          {selectedCompany.traderTier}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-gray-200 dark:border-slate-700 pt-3 space-y-1.5">
                      <div className="flex items-center gap-2 text-[10px] text-gray-500">
                        ...
                        {selectedCompany.contacts.phone}
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-gray-500">
                        <Mail className="w-3 h-3" />
                        {selectedCompany.contacts.email}
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-gray-500">
                        <Globe className="w-3 h-3" />
                        {selectedCompany.contacts.website}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 mt-4">
                      ...
                      <button className="px-3 py-2 bg-gray-100 dark:bg-slate-700 rounded-lg text-[10px] font-bold text-gray-700 dark:text-gray-300">
                        Audit
                      </button>
                      <button className="px-3 py-2 bg-gray-100 dark:bg-slate-700 rounded-lg text-[10px] font-bold text-gray-700 dark:text-gray-300">
                        Flag
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4">
                    ...
                    <div className="space-y-2">
                      {selectedCompany.history.map((h, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/50"
                        >
                          <div
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-8 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-[10px] text-gray-300 mt-1">
                    CRM-style profile with ownership, history & compliance
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-3 text-[10px]">
                  <span className="flex items-center gap-1">
                    <div className="w-2 h-5 rounded bg-green-500"></div> Supplier
                  </span>
                  <span className="flex items-center gap-1">
                    <div className="w-2 h-5 rounded bg-blue-500"></div> Buyer
                  </span>
                  <span className="flex items-center gap-1">
                    <div className="w-2 h-5 rounded bg-purple-500"></div> Logistics
                  </span>
                  <span className="flex items-center gap-1">
                    <div className="w-2 h-5 rounded bg-red-500"></div> Suspicious
                  </span>
                </div>
              </div>
                  ...
                  {NETWORK_LINKS.map((link, i) => {
                    ...
                    return (
                      <line
                        ...
                        strokeWidth={
                          link.strength === 'strong' ? 3 : link.strength === 'suspicious' ? 3 : 1.5
                        }
              ...
              <div className="space-y-2">
                {NETWORK_LINKS.map((link, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-gray-100 dark:bg-slate-700 rounded text-[10px] font-bold capitalize">
                        {link.type.replace('_', ' ')}
                      </span>
                      <span
                    ...
                    <div className="flex items-center justify-center mb-3">
                      <div className="relative w-20 h-20">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <p
                            className={`text-lg font-black ${getScoreColor(company.complianceScore)}`}
                          >
                            {company.complianceScore}
                          </p>
                        </div>
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-5 rounded-xl border border-blue-200 dark:border-blue-800">
              ...
              <div className="space-y-2">
                {[
                  {
                    entity: 'MadaTrade SARL',
                    license: 'Export License',
                    expires: '2025-01-15',
                    daysLeft: 28,
                  },
                  {
                    entity: 'Trans-Sahel Logistics',
                    license: 'Transit Permit',
                    expires: '2025-02-01',
                    daysLeft: 45,
                  },
                  {
                    entity: 'Green Valley Farms',
                    license: 'SPS Certificate',
                    expires: '2024-12-30',
                    daysLeft: 12,
                  },
                ].map((r, i) => (
                  <div
                    key={i}
                    className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-blue-200 dark:border-blue-700 flex items-center justify-between"
                  >
                    <div>
                      <p className="text-xs font-bold text-gray-900 dark:text-white">{r.entity}</p>
                      <p className="text-[10px] text-gray-500">
                        {r.license} &middot; Expires: {r.expires}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold ${r.daysLeft <= 14 ? 'bg-red-100 text-red-700' : r.daysLeft <= 30 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}
                      >
                        {r.daysLeft} days
                      </span>
                      <button className="px-2 py-1 bg-blue-600 text-white rounded text-[9px] font-bold">
                        Notify
                      </button>
                    </div>
                  </div>
