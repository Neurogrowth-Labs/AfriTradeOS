...
const WIZARD_STEPS = [
  { step: 1, title: 'Entity Information', description: 'Basic details & registration', icon: User },
  {
    ...
    description: 'ID, registration, tax documents',
    icon: Upload,
  },
  { step: 3, title: 'AML/Sanctions Check', description: 'Automated screening', icon: Shield },
  {
    ...
    description: 'Officer review & approval',
    icon: CheckCircle,
  },
...
const CROSS_COUNTRY_VERIFICATIONS = [
  {
    id: 'XV-001',
    entity: 'Trans-Africa Mining',
    requestedBy: 'Ghana Customs',
  ...
  {
    id: 'XV-002',
    entity: 'EastWest Traders',
    requestedBy: 'Kenya Revenue',
  ...
  {
    id: 'XV-003',
    entity: 'Sahara Freight Co',
    requestedBy: 'Morocco Customs',
  ...
  {
    id: 'XV-004',
    entity: 'Lagos Pharma Ltd',
    requestedBy: 'NAFDAC Nigeria',
    targetCountry: 'India',
    status: 'rejected',
    responseTime: '24h',
  },
...
export const GovEntityVerification: React.FC = () => {
  ...
  const [KYC_APPLICATIONS, setKycApplications] = useState<
    {
      id: string;
      entity: string;
      type: string;
  ...
  const [TRUSTED_TIERS, setTrustedTiers] = useState<
    {
      tier: string;
      label: string;
      count: number;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setKycApplications(
          kycRequests.map((k: Record<string, unknown>, i: number) => {
            ...
            return {
              ...
              entity:
                (k.entity_name as string) || (k.user_id as string)?.substring(0, 8) || 'Unknown',
              type: (k.request_type as string) === 'kyb' ? 'Organization' : 'Individual',
              country: (k.country as string) || 'Unknown',
              submittedDate: k.submitted_at
                ? new Date(k.submitted_at as string).toLocaleDateString()
                : '',
              status: statusMap[k.status as string] || 'pending',
              riskLevel: riskScore > 70 ? 'high' : riskScore > 40 ? 'medium' : 'low',
              documents: ((k.documents_submitted as string[]) || []).length,
              completeness:
                (k.status as string) === 'approved' ? 100 : Math.round(Math.random() * 40 + 50),
        ...
        setTrustedTiers([
          ...
          },
          {
            ...
            benefits: ['Simplified procedures', 'Reduced bond requirements', 'Periodic review'],
            minScore: 75,
            color: '#94a3b8',
          },
          {
            ...
            benefits: ['Standard processing', 'Online submissions'],
            minScore: 50,
            color: '#b45309',
          },
          {
            ...
            benefits: ['Basic access', 'Manual processing'],
            minScore: 0,
            color: '#6b7280',
          },
        ]);
  ...
  const filteredApps = KYC_APPLICATIONS.filter(app => {
    const matchesSearch =
      searchQuery === '' ||
      app.entity.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Trust framework — Digital KYC, AML screening, sanctions checks & trusted trader tiers
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="bg-amber-50 dark:bg-amber-900/10 p-3 rounded-xl border border-amber-200 dark:border-amber-800">
          ...
          <p className="text-xl font-black text-amber-600">
            {KYC_APPLICATIONS.filter(a => a.status === 'pending').length}
          </p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-gray-100 dark:border-slate-700">
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-slate-700 rounded-lg text-xs border-none outline-none"
                  />
                </div>
                <select
                  value={statusFilter}
                  ...
                  <div
                    ...
                    className={`p-3 rounded-lg border cursor-pointer transition-all ${
                      selectedApp?.id === app.id
                        ? 'bg-trade-primary/5 border-trade-accent'
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center ${app.type === 'Individual' ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-purple-100 dark:bg-purple-900/30'}`}
                        >
                          {app.type === 'Individual' ? (
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400">
                            {app.id} &middot; {app.country} &middot; {app.submittedDate}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span
                          className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${getRiskStyle(app.riskLevel)}`}
                        >
                          {app.riskLevel}
                        </span>
                        <span
                          className={`px-1.5 py-0.5 rounded text-[9px] font-bold capitalize ${getStatusStyle(app.status)}`}
                        >
                    ...
                    <div className="mt-2 flex items-center gap-3">
                      <div className="flex-1 h-1.5 bg-gray-200 dark:bg-slate-700 rounded-full">
                        <div
                          className={`h-full rounded-full ${app.completeness === 100 ? 'bg-green-500' : app.completeness >= 70 ? 'bg-amber-500' : 'bg-red-500'}`}
                  ...
                  {WIZARD_STEPS.map(ws => (
                    <div
                      ...
                      className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-all ${
                        wizardStep === ws.step
                          ? 'bg-trade-primary/10 border border-trade-accent'
                          : wizardStep > ws.step
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4">
                  ...
                  <div className="space-y-2 text-xs mb-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono font-bold text-trade-primary dark:text-trade-accent">
                        {selectedApp.id}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-gray-900 dark:text-white">
                        {selectedApp.assignee}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-500">Risk</span>
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold ${getRiskStyle(selectedApp.riskLevel)}`}
                      >
                  ...
                  {/* Quick Actions */}
                  <div className="flex gap-2">
                    <button className="flex-1 px-3 py-2 bg-green-600 text-white rounded-lg text-[10px] font-bold hover:bg-green-700">
                      Approve
                    </button>
                    <button className="flex-1 px-3 py-2 bg-red-600 text-white rounded-lg text-[10px] font-bold hover:bg-red-700">
                      Reject
                    </button>
                    <button className="px-3 py-2 bg-amber-500 text-white rounded-lg text-[10px] font-bold hover:bg-amber-600">
                      Escalate
                    </button>
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-100 dark:border-slate-700">
              ...
              <div className="flex gap-2 mb-4">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Enter entity name, ID, or passport number..."
                ...
                {AML_SCREENING.map((screen, i) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border flex items-center justify-between ${
                      screen.status === 'clear'
                        ? 'bg-green-50 dark:bg-green-900/5 border-green-200 dark:border-green-800'
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">
                          Last checked: {screen.lastChecked}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      {screen.status === 'review' && (
                        <button className="px-2 py-1 bg-amber-600 text-white rounded text-[9px] font-bold">
                          Review
                        </button>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-5 rounded-xl border border-blue-200 dark:border-blue-800">
                ...
                <p className="text-xs text-blue-700 dark:text-blue-400 mb-3">
                  Advanced identity verification using biometric matching
                </p>
                <div className="space-y-2">
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 p-5 rounded-xl border border-purple-200 dark:border-purple-800">
                ...
                <p className="text-xs text-purple-700 dark:text-purple-400 mb-3">
                  Immutable identity records on distributed ledger
                </p>
                <div className="space-y-2">
          ...
          <div className="space-y-4 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {TRUSTED_TIERS.map(tier => (
                <div
                  key={tier.tier}
                  className="bg-white dark:bg-slate-800 p-5 rounded-xl border-2 hover:shadow-md transition-shadow"
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 p-5 rounded-xl border border-green-200 dark:border-green-800">
              ...
              <p className="text-xs text-green-700 dark:text-green-400 mb-3">
                Traders earn badges for sustained compliance. Higher tiers unlock benefits.
              </p>
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <span
                    key={badge}
                    className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-green-200 dark:border-green-700 rounded-full text-[10px] font-bold text-green-700 dark:text-green-400 flex items-center gap-1"
                  >
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-100 dark:border-slate-700">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-gray-900 dark:text-white text-sm flex items-center gap-2">
                  <Globe className="w-4 h-4 text-trade-accent" /> Cross-Country Verification
                  Requests
                </h3>
                <button className="px-3 py-1.5 bg-trade-primary text-white rounded-lg text-[10px] font-bold">
                  New Request
                </button>
              </div>
                    ...
                    {CROSS_COUNTRY_VERIFICATIONS.map(v => (
                      <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${getStatusStyle(v.status)}`}
                          >
                            {v.status}
                          </span>
                        </td>
