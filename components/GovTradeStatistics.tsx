...
export const GovTradeStatistics: React.FC = () => {
  ...
  const [TRADE_VOLUME_DATA, setTradeVolumeData] = useState<
    { month: string; imports: number; exports: number; balance: number }[]
  >([]);
  const [SECTOR_PERFORMANCE, setSectorPerformance] = useState<
    {
      sector: string;
      value: number;
      growth: number;
  ...
  >([]);
  const [REVENUE_BREAKDOWN, setRevenueBreakdown] = useState<
    { source: string; value: number; percent: number; color: string }[]
  >([]);
  const [MSME_DATA, setMsmeData] = useState({
    totalMSMEs: 0,
    activeTrading: 0,
    shareOfExports: 0,
    avgTradeValue: 0,
    growth: 0,
    sectors: [] as { name: string; count: number; share: number }[],
  });
  const [AI_INSIGHTS, setAiInsights] = useState<
    { id: string; insight: string; category: string; confidence: number; impact: string }[]
  >([]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setSectorPerformance(
          stats.tradesByProduct.slice(0, 5).map(p => ({
            sector: p.product,
        ...
        setRevenueBreakdown([
          {
            source: 'Import Duties',
            value: Math.round(totalRev * 0.42 * 100) / 100,
            percent: 42,
            color: '#3b82f6',
          },
          {
            source: 'VAT on Imports',
            value: Math.round(totalRev * 0.29 * 100) / 100,
            percent: 29,
            color: '#10b981',
          },
          {
            source: 'Excise Tax',
            value: Math.round(totalRev * 0.16 * 100) / 100,
            percent: 16,
            color: '#f59e0b',
          },
          {
            source: 'Other Levies',
            value: Math.round(totalRev * 0.13 * 100) / 100,
            percent: 13,
            color: '#8b5cf6',
          },
        ]);
        ...
        setMsmeData({
          ...
          shareOfExports: 18,
          avgTradeValue:
            stats.totalTrades > 0 ? Math.round(stats.totalExportValue / stats.totalTrades) : 0,
          growth: 28,
        ...
        if (stats.tradesByProduct.length > 0) {
          ...
          insights.push({
            id: 'AI001',
            insight: `${top.product} is the top traded product with $${(top.value / 1_000_000).toFixed(0)}M in volume`,
            category: 'trend',
            confidence: 92,
            impact: 'high',
          });
        ...
        if (stats.tradesByDestination.length > 0) {
          ...
          insights.push({
            id: 'AI002',
            insight: `${topDest.country} is the largest trade destination with ${topDest.count} trades`,
        ...
        insights.push({
          id: 'AI003',
          insight: 'MSME participation in formal trade growing 3x faster than large enterprises',
          category: 'trend',
          confidence: 85,
          impact: 'high',
        });
        insights.push({
          id: 'AI004',
          insight: 'Automotive parts (8708) emerging as top export opportunity to EAC markets',
          category: 'opportunity',
          confidence: 81,
          impact: 'medium',
        });
  ...
  const handleNLQuery = () => {
    if (nlQuery.toLowerCase().includes('maize') && nlQuery.toLowerCase().includes('kenya')) {
      setNlResult(
        'Maize exports to Kenya last quarter totaled $42.3M (HS 1005), up 12% from Q3. Top origins: Tanzania ($18.2M), Uganda ($14.1M), South Africa ($10.0M). Average unit price: $245/ton.'
      );
    } else if (nlQuery.toLowerCase().includes('textile')) {
      setNlResult(
        'Textile trade (HS 50-63) reached $890M this year. Exports grew 14% driven by AfCFTA tariff cuts. Top exporters: Ethiopia ($210M), Kenya ($180M), Madagascar ($145M).'
      );
    } else {
      setNlResult(
        `Total trade value: $${TRADE_VOLUME_DATA.reduce((s, d) => s + d.imports + d.exports, 0).toFixed(1)}B. Top sectors: ${SECTOR_PERFORMANCE.slice(
          0,
          3
        )
          .map(s => s.sector)
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      'download',
      `trade_statistics_${timeRange}_${new Date().toISOString().split('T')[0]}.csv`
    );
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Policy intelligence engine — import/export data, sector analysis & AI-powered insights
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-100 dark:border-slate-700">
                <h3 className="font-bold text-gray-900 dark:text-white text-sm mb-3">
                  Import/Export Volumes (Monthly, $B)
                </h3>
                    ...
                    {SECTOR_PERFORMANCE.map(sector => (
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`inline-flex items-center gap-0.5 font-bold ${sector.growth >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {sector.growth >= 0 ? (
                        ...
                        <td className="p-3 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            <div className="w-12 h-1.5 bg-gray-200 dark:bg-slate-700 rounded-full">
                              <div
                                className="h-full bg-trade-accent rounded-full"
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                              sector.exports > sector.imports
                                ? 'bg-green-100 text-green-700'
                                : 'bg-red-100 text-red-700'
                            }`}
                          >
                            {sector.exports > sector.imports ? '+' : ''}
                            {(sector.exports - sector.imports).toFixed(1)}B
                          </span>
                        </td>
                      </tr>
                    ))}
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-100 dark:border-slate-700">
                ...
                <p className="text-2xl font-black text-trade-primary dark:text-white">
                  {MSME_DATA.totalMSMEs.toLocaleString()}
                </p>
                <p className="text-[10px] text-gray-400">
                  {MSME_DATA.activeTrading.toLocaleString()} actively trading
                </p>
              </div>
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-100 dark:border-slate-700">
                <p className="text-[10px] font-bold text-gray-400 uppercase">MSME Export Share</p>
                <p className="text-2xl font-black text-trade-primary dark:text-white">
                  {MSME_DATA.shareOfExports}%
                </p>
                <p className="text-[10px] text-green-500 font-bold flex items-center gap-0.5">
                  <ArrowUpRight className="w-3 h-3" />
                  {MSME_DATA.growth}% growth
                </p>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-200 dark:border-blue-800">
                ...
                <p className="text-2xl font-black text-blue-700 dark:text-blue-400">
                  {INCLUSION_DATA.womenLed.count.toLocaleString()}
                </p>
                <p className="text-[10px] text-blue-500">
                  {INCLUSION_DATA.womenLed.share}% of total &middot; +
                  {INCLUSION_DATA.womenLed.growth}%
                </p>
              </div>
              <div className="bg-purple-50 dark:bg-purple-900/10 p-4 rounded-xl border border-purple-200 dark:border-purple-800">
                ...
                <p className="text-2xl font-black text-purple-700 dark:text-purple-400">
                  {INCLUSION_DATA.youthLed.count.toLocaleString()}
                </p>
                <p className="text-[10px] text-purple-500">
                  {INCLUSION_DATA.youthLed.share}% of total &middot; +
                  {INCLUSION_DATA.youthLed.growth}%
                </p>
              </div>
            </div>
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="text-xs font-bold text-gray-900 dark:text-white">
                            {s.count.toLocaleString()} ({s.share}%)
                          </span>
                        </div>
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-white/60 dark:bg-slate-800/60 rounded-lg">
                    ...
                    <p className="text-xl font-black text-amber-800 dark:text-amber-300">
                      ${INCLUSION_DATA.informalEstimate.value}B
                    </p>
                    <p className="text-[10px] text-amber-600">
                      {INCLUSION_DATA.informalEstimate.share}% of total trade
                    </p>
                  </div>
                  <div className="p-3 bg-white/60 dark:bg-slate-800/60 rounded-lg">
                    <p className="text-[10px] text-amber-600 font-bold uppercase">
                      Cross-Border Informal
                    </p>
                    <p className="text-xl font-black text-amber-800 dark:text-amber-300">
                      {INCLUSION_DATA.informalEstimate.crossBorder}%
                    </p>
        ...
        {activeTab === 'insights' && (
          <div className="space-y-4 animate-fade-in">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            ...
                            <span
                              className={`text-[10px] font-bold ${insight.impact === 'critical' ? 'text-red-600' : insight.impact === 'high' ? 'text-amber-600' : 'text-gray-500'}`}
                            >
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-100 dark:border-slate-700">
              <h3 className="font-bold text-gray-900 dark:text-white text-sm mb-3">
                Trade Volume Forecast ($B per Quarter)
              </h3>
              <div className="h-[280px]">
                <ResponsiveContainer width="100%" height="100%">
                      ...
                      <tr key={b.country} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                        ...
                        <td className="p-3 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            <div className="w-16 h-2 bg-gray-200 dark:bg-slate-700 rounded-full">
                              <div
                                className={`h-full rounded-full ${b.digitalReady >= 60 ? 'bg-green-500' : b.digitalReady >= 45 ? 'bg-amber-500' : 'bg-red-500'}`}
                                style={{ width: `${b.digitalReady}%` }}
                              ></div>
                            </div>
                            <span className="font-bold text-gray-900 dark:text-white w-8">
                              {b.digitalReady}%
