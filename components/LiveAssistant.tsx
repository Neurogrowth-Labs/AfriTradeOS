...
export const LiveAssistant: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10">
      <div
        className={`relative overflow-hidden rounded-3xl border transition-all duration-500 ${active ? 'bg-trade-primary border-indigo-500 shadow-2xl shadow-indigo-500/20' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 shadow-lg'}`}
      >
        <div className="p-8 flex flex-col items-center justify-center min-h-[400px]">
          {/* Visualizer Circle */}
          <div
            className={`relative mb-8 w-32 h-32 rounded-full flex items-center justify-center transition-all duration-300 ${active ? 'bg-indigo-500/10' : 'bg-gray-100 dark:bg-slate-700'}`}
          >
            {active && !isMuted && (
              <>
          ...
          <h2
            className={`text-2xl font-bold mb-2 ${active ? 'text-white' : 'text-trade-primary dark:text-white'}`}
          >
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium flex items-center gap-2 transition-all ${
                  status === 'Listening...'
                    ...
                    : status === 'Speaking...'
                      ...
                      : status === 'Processing audio...' || status === 'Awaiting response...'
        ...
        <div
          className={`border-t p-6 ${active ? 'bg-slate-800/50 border-slate-700' : 'bg-gray-50 dark:bg-slate-900/50 border-gray-100 dark:border-slate-700'}`}
        >
          <div className="flex items-center gap-2 mb-3">
            <Volume2 className={`w-4 h-4 ${active ? 'text-indigo-400' : 'text-gray-400'}`} />
            <span
              className={`text-xs font-semibold uppercase tracking-wider ${active ? 'text-indigo-400' : 'text-gray-400'}`}
            >
              Live Transcript
            </span>
          </div>
          <div className="space-y-2 h-32 overflow-y-auto custom-scrollbar">
            {transcript.length === 0 && (
              <p
                className={`text-sm italic ${active ? 'text-slate-500' : 'text-gray-400 dark:text-gray-500'}`}
              >
            ...
            {transcript.map((msg, i) => (
              <p
                key={i}
                className={`text-sm ${msg.role === 'user' ? 'text-slate-500' : active ? 'text-slate-200' : 'text-trade-primary dark:text-gray-200'}`}
              >
                <span className="font-bold opacity-50">{msg.role === 'user' ? 'You' : 'AI'}:</span>{' '}
                {msg.text}
              </p>
            ))}
          </div>
