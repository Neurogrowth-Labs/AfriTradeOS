      ...
      try {
        const fallbackText = await callOpenRouter(
          [
            {
              role: 'system',
              content: `You are an expert AfCFTA trade lawyer. Provide concise compliance analysis.`,
            },
            {
              role: 'user',
              content: `Analyze the following trade scenario (provide a concise analysis):\n\n${scenario}`,
            },
          ],
          'google/gemini-2.0-flash-001'
        );
...
export const connectLiveSession = async (
  ...
  console.warn(
    'Live voice session not fully supported via OpenRouter. Using simplified text-based interaction.'
  );
